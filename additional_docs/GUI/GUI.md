...
Guizero はオブジェクト (これも部品のこと) を使用して、Raspberry Pi上でグラフィカルユーザーインターフェース（GUI）を開発できるようにする。ここでは、組み込みオブジェクトの大部分で十分である。
ただし、より高度な GUI を開発するには、大量のライブラリが必要になる場合がある。GUIの開発に使用できるオブジェクトのリストを以下に示す。
...
LED を 1 秒間隔で点滅させる従来のコードは次のようになる。
```python
def blink_LED():
  while True:
    myled.on()
